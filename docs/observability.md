...
- `qualyra_photo_evidence_capacity_tenants` reports only aggregate tenant counts in normal, warning, critical, over-quota, and counter-mismatch states. Reconciliation run, duration, and last-success metrics expose worker health without tenant labels.
